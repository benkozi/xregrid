...
def create_sample_dataset(name, nlat=45, nlon=90, lat_range=(-90, 90), lon_range=(0, 360)):
...
print(f"Original data range: [{source_ds.temperature.min().values:.3f}, {source_ds.temperature.max().values:.3f}]")
print(f"Regridded data range: [{regridded_temp.min().values:.3f}, {regridded_temp.max().values:.3f}]")
